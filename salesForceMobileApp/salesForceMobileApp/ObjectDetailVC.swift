...
//  salesForceMobileApp
//
//  Created by KloudRac on 16/01/17.
...
class ObjectDetailVC: UITableViewController, SFRestDelegate, ExecuteQueryDelegate, UIActionSheetDelegate,UpdateInfoDelegate {
    ...
    @IBAction func leadAction(sender: AnyObject) {
        if leadSegment.selectedSegmentIndex == 0 {
            dispatch_async(dispatch_get_main_queue(), {
                let path: String =  "/services/data/v36.0/sobjects/\(self.objectType!.rawValue)/\(self.objectID)/feeds"
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        if isUpdatedSuccessfully {
            if exDelegate.isConnectedToNetwork() {
                exDelegate.leadQueryDe(objectType!.rawValue)
            }
    ...
    override func viewDidLoad() {
        ...
        title = "\(objectType!.rawValue) Detail"
        ...
        let navBarActionBtn: UIBarButtonItem = UIBarButtonItem(barButtonSystemItem: .Action, target: self, action: #selector(ObjectDetailVC.shareAction))
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        if leadSegment.selectedSegmentIndex == 1 {
            ...
            if fileContentName == nil {
                ...
                textFeedCell.shareText.text = self.feedData.objectAtIndex(indexPath.row)["Body"] as?
                String
                let tapGesture = UITapGestureRecognizer(target: self, action: #selector(ObjectDetailVC.tapOnImage(_:)))
    ...
    func editAction() {
        ...
        nv.objectType = objectType!.rawValue
